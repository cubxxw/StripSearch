...
打开 <http://localhost:4392>。服务器只绑定 loopback，不支持远程主机。
...
| `STRIPSEARCH_PUBLIC_ORIGIN` | 受信任的同源地址，必须是 loopback。 |
...
| 启动限流 | 每用户 60 秒 10 次 |
| 记录上限 | 每用户 200 份研究 |
| 会话 | 7 天，HttpOnly、SameSite=Lax，cookie 前缀 `stripsearch` |
...
- 离线自动化测试 63 项通过；另行真实验收完成 GitHub 调研（2 次请求、9 条来源）、账号隔离、导出、修订和重启持久化。记录仅保留状态与计数，不提交人物档案。
